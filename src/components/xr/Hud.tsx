...
interface HudProps {
    setOrbitControl: (value: boolean) => void;
    setPhotoIndex: (index: number) => void;
    mediaPath: string;
    mediaDb: any[];
}

const Hud: FC<HudProps> = ({ setOrbitControl, setPhotoIndex, mediaPath, mediaDb }) => {
    const [isVr, setIsVr] = useState<boolean>(false);
    const { gl, camera } = useThree();
    let pre: { position: number[]; rotation: number[] } = {
        position: [0, 0, -20],
        rotation: [0, 0, 1],
    };
    ...
    let xp: number, yp: number, zp: number, xr: number, yr: number, zr: number;
